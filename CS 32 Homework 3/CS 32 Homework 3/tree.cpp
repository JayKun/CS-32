...
{
	if (n2 < 1) // if you find the sequence a2 in a1
		return 1; 
	if (n1 < 1) // if the sequence a2 does not exists in a1
		return 0; 
	if (a2[0] == a1[0]) 
...
{	
	...
	if (firstNotGreater > 1) // there are still items that are yet to be sorted
		order(a, firstNotGreater); 
	if (firstLess < n - 1)	// there are still items to be sorted
